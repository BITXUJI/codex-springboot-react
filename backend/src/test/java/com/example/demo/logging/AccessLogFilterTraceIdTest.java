...
class AccessLogFilterTraceIdTest {
  ...
  void resolveTraceIdUsesHeader() {
    ...
    Assertions.assertEquals(
        TRACE_VALUE, AccessLogFilter.resolveTraceId(withTrace), "Trace id header should be reused");
  }
  ...
  void resolveTraceIdGeneratesWhenMissing() {
    final MockHttpServletRequest withoutTrace = new MockHttpServletRequest(METHOD_GET, PATH_API);

    Assertions.assertNotNull(
        AccessLogFilter.resolveTraceId(withoutTrace), "Trace id should be generated when missing");
  }
}
