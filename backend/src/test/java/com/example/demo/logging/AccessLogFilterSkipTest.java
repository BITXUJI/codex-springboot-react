...
class AccessLogFilterSkipTest {
  ...
  void shouldSkipActuatorPath() {
    ...
    final MockHttpServletRequest request = new MockHttpServletRequest(METHOD_GET, PATH_ACTUATOR);

    Assertions.assertTrue(filter.shouldNotFilter(request), "Actuator requests should be skipped");
  }
  ...
  void shouldNotSkipApiPath() {
    ...
    Assertions.assertFalse(filter.shouldNotFilter(request), "API requests should not be skipped");
  }
  ...
  void shouldNotSkipBlankPath() {
    ...
    Assertions.assertFalse(filter.shouldNotFilter(request), "Blank paths should not be skipped");
  }
}
