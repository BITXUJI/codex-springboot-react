...
class AccessLogFilterFilterTest {
  ...
  void doFilterInternalCopiesResponseBody() throws IOException, ServletException {
    ...
    filter.doFilterInternal(
        request,
        response,
        (req, res) -> res.getOutputStream().write(RESPONSE_BODY.getBytes(StandardCharsets.UTF_8)));

    Assertions.assertEquals(
        RESPONSE_BODY, response.getContentAsString(), "Response body should be preserved");
  }
  ...
  void doFilterInternalGeneratesTraceIdWhenMissing() throws IOException, ServletException {
    ...
    Assertions.assertNotNull(
        response.getHeader(TRACE_HEADER), "Trace id should be generated when missing");
  }
