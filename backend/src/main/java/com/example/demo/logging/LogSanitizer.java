...
public final class LogSanitizer {
  ...
  private static final Pattern JSON_MASK =
      Pattern.compile(
          "(?i)(\\\"(?:password|passwd|pwd|secret|token|access_token|refresh_token"
  ...
  private static final Pattern FORM_MASK =
      Pattern.compile(
          "(?i)\\b(password|passwd|pwd|secret|token|access_token|refresh_token"
              + "|authorization|auth|apiKey|apikey|cardNumber|creditCard|ssn|idCard)"
              + "=([^&\\s]+)");
