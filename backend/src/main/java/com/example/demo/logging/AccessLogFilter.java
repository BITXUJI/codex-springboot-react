...
public class AccessLogFilter extends OncePerRequestFilter {
  ...
  protected void logAccess(
    ...
    final BodyCapture requestBody = captureBody(request.getContentAsByteArray(), reqContentType);
    final BodyCapture responseBody = captureBody(response.getContentAsByteArray(), respContentType);
  ...
  protected static String resolveClientIp(final HttpServletRequest request) {
    ...
    if (StringUtils.hasText(forwarded)) {
      ...
      clientIp = commaIndex > 0 ? forwarded.substring(0, commaIndex).trim() : forwarded.trim();
    }
    return clientIp;
  }
